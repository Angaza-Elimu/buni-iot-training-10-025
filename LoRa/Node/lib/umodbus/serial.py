...
class ModbusRTU(Modbus):
    ...
    :type       ctrl_pin:    int
    :param      de_pin:      The transmit control pin
    :type       de_pin:      int
    :param      re_pin:      The receive control pin
    :type       re_pin:      int
...
class Serial(CommonModbusFunctions):
    def __init__(self,
        ...
        if de_pin is not None:
            self._de_pin = Pin(de_pin, Pin.OUT)
        else:
            self._de_pin = None

        if re_pin is not None:
            self._re_pin = Pin(re_pin, Pin.OUT)
        else:
            self._re_pin = None
    ...
    def _send(self, modbus_pdu: bytes, slave_addr: int) -> None:
        ...
        if self._de_pin and self._re_pin:
            self._re_pin.value(1)
            self._de_pin.value(1)
            time.sleep_us(200)
        ...
        # print(f'modbus adu: {modbus_adu}')
        # print(f'modbus adu hex: {modbus_adu.hex()}')
        ...
        #     self._ctrlPin.off()
        if self._de_pin and self._re_pin:
            self._de_pin.value(0)
            self._re_pin.value(0)
            # print(f'DE: {self._de_pin.value()}, RE: {self._re_pin.value()}')
            time.sleep_us(200)
    ...
    def _send_receive(self,
        ...
        # print(f'sent modbus pdu: {modbus_pdu}')
    ...
    def _validate_resp_hdr(self,
        ...
        # print(f'response returned: {response[hdr_length:len(response) - Const.CRC_LENGTH]}')
