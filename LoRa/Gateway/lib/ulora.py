...
class LoRa(object):
    def __init__(self, spi_channel, interrupt, this_address, cs_pin, reset_pin=None, freq=868.0, tx_power=14,
        ...
        reset_pin: the GPIO used to reset the RFM9x or RA-02 if connected
...
con_dict = {
    ...
    'COMBINED': 'cmbd',
    'AMBIENT_LIGHT': 'a_l',
    'DHT_TEMPERATURE': 'dh_t',
    'DHT_HUMIDITY': 'dh_h',
    'SOIL_TEMPERATURE': 's_t',
    'SOIL_MOISTURE': 's_m',
    'TDS': 'tds',
    'Nitrogen': 'n_ch',
    'Phosphorus': 'p_ch',
...
# This function constricts/expands keys and values of a dict
def conex_dict(provided_dict, ref_dict):
    conexed_dict = {}

    for key, value in provided_dict.items():
        temp_key = ref_dict.get(key, None)
        temp_value = ref_dict.get(str(value), None)
        temp_key = key if temp_key is None else temp_key
        temp_value = value if temp_value is None else temp_value
        conexed_dict[temp_key] = temp_value
