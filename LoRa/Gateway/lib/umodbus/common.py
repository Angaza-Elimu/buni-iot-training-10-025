...
class CommonModbusFunctions(object):
    ...
    def read_slave_address(self):
        return self.read_holding_registers(255, 15, 1)        
    ...
    def write_single_register(self,
        ...
        :rtype:     bool
        """
        write_slave_addr = kwargs.get('write_slave_addr', False)
    ...
    def write_slave_address(self, new_slave_address):
        if new_slave_address < 1 or new_slave_address > 254:
            raise Exception('Slave address must be from 1 to 254')
        current_slave_address = self.read_slave_address()
        if not self.write_single_register(current_slave_address, 15, new_slave_address, write_slave_addr=True):
